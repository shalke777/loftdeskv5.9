...
const seedState: DemoState = {
  ...
  projects: [
    ...
    { id: 'p2', company_id: 'cmp-wisniewski', client_id: 'c2', number: 'PRJ/2026/002', name: 'Remont łazienki – Kowalski', status: 'done', start_date: dateOnly(), end_date: null, address: 'Warszawa, ul. Słoneczna 5', budget: 17000, costs: 9100, notes: 'Oferta wygrana, projekt gotowy do fakturowania.', created_at: now() },
  ],
  estimates: [
...
function normalizeState(state: DemoState): DemoState {
  return {
    ...
    invoices: (state.invoices ?? []).map((item) => { const items = normalizeInvoiceItems(item.items); const totals = calcInvoiceTotals(items as InvoiceItem[]); return { ...item, contract_id: item.contract_id ?? null, items, total_net: Number(item.total_net ?? totals.totalNet), total_gross: Number(item.total_gross ?? totals.totalGross) } }),
    ...
    projects: (state.projects ?? []).map((item) => ({ ...item, costs: Number(item.costs ?? 0) })),
...
export const demoDb = {
  ...
  invoices: {
    ...
    createFromEstimate(companyId: string, estimateId: string) { const state = read(); const estimate = state.estimates.find((item) => item.id === estimateId && item.company_id === companyId); if (!estimate) throw new Error('Estimate not found'); return demoDb.invoices.create({ company_id: companyId, client_id: estimate.client_id, project_id: null, contract_id: null, status: 'unpaid', issue_date: dateOnly(), due_date: plusDays(7).slice(0,10), ksef_status: 'ksef_pending', ksef_ref: null, notes: `Wygenerowano z kosztorysu ${estimate.number}`, items: estimate.items.map((item, index) => ({ id: crypto.randomUUID(), description: item.name + (item.description ? ` — ${item.description}` : ''), unit: item.unit, quantity: item.quantity, unit_price: item.unit_price, vat_rate: item.vat_rate, sort_order: index + 1, tranche_label: '' })) }) },
    createFromProject(companyId: string, config: { projectId: string; vatRate?: number; tranches?: Array<{ id: string; label: string; amount: number; due_date: string }> }) { const { projectId, vatRate = 23, tranches = [] } = config; const state = read(); const project = state.projects.find((item) => item.id === projectId && item.company_id === companyId); if (!project) throw new Error('Project not found'); const totalValue = Number(project.budget ?? 0) || 0; const items = tranches.length > 0 ? tranches.map((tr, idx) => ({ id: crypto.randomUUID(), description: tr.label || `Transza ${idx + 1}`, unit: 'kpl', quantity: 1, unit_price: tr.amount, vat_rate: vatRate, sort_order: idx + 1, tranche_label: tr.label || `Transza ${idx + 1}` })) : [{ id: crypto.randomUUID(), description: `Rozliczenie projektu ${project.name}`, unit: 'kpl', quantity: 1, unit_price: totalValue, vat_rate: vatRate, sort_order: 1, tranche_label: '' }]; return demoDb.invoices.create({ company_id: companyId, client_id: project.client_id, project_id: project.id, contract_id: null, status: 'unpaid', issue_date: dateOnly(), due_date: plusDays(7).slice(0,10), ksef_status: 'ksef_pending', ksef_ref: null, notes: `Faktura z projektu ${project.number}`, items }) },
  },
  contracts: {
    ...
    create(input: Omit<Contract, 'id' | 'number' | 'created_at'>) { const state = read(); const record: Contract = { ...input, id: crypto.randomUUID(), number: nextNumber('UMW', state.contracts.filter((x) => x.company_id === input.company_id).length), created_at: now(), tranches: normalizeTranches(input.tranches) }; state.contracts.unshift(record); writeState(state); return record },
  ...
  projects: {
    ...
    create(input: Omit<Project, 'id' | 'number' | 'created_at'>) { const state = read(); const record: Project = { ...input, id: crypto.randomUUID(), number: nextNumber('PRJ', state.projects.filter((x) => x.company_id === input.company_id).length), created_at: now(), costs: Number(input.costs ?? 0) }; state.projects.unshift(record); writeState(state); return record },
    ...
    createFromEstimate(companyId: string, estimateId: string) { const state = read(); const estimate = state.estimates.find((item) => item.id === estimateId && item.company_id === companyId); if (!estimate) throw new Error('Estimate not found'); return demoDb.projects.create({ company_id: companyId, client_id: estimate.client_id, name: estimate.name, status: 'offer', start_date: null, end_date: null, address: '', budget: estimate.total_gross, costs: Math.round(estimate.total_net * 0.65), notes: `Projekt utworzony z kosztorysu ${estimate.number}` }) },
