...
const seedState: DemoState = {
  ...
  invoices: [
    ...
    { id: 'f2', company_id: 'cmp-wisniewski', client_id: 'c2', project_id: null, contract_id: null, number: 'FV/2026/002', status: 'unpaid', issue_date: dateOnly(), sale_date: dateOnly(), due_date: plusDays(14).slice(0,10), total_net: 4500, total_gross: 4860, ksef_status: 'ksef_pending', ksef_ref: null, notes: 'Malowanie klatki schodowej', created_at: now(), items: [{ id: 'ii2', description: 'Malowanie ścian klatki schodowej', unit: 'm²', quantity: 180, unit_price: 25, vat_rate: 8, sort_order: 1, tranche_label: '' }] },
    { id: 'f3', company_id: 'cmp-wisniewski', client_id: 'c1', project_id: 'p1', contract_id: 'u1', number: 'FV/2026/003', status: 'unpaid', issue_date: dateOnly(), sale_date: dateOnly(), due_date: plusDays(14).slice(0,10), total_net: 6500, total_gross: 7995, ksef_status: 'ksef_pending', ksef_ref: null, notes: 'Etap II – instalacja elektryczna', created_at: now(), items: [{ id: 'ii3', description: 'Instalacja elektryczna – etap II', unit: 'kpl', quantity: 1, unit_price: 6500, vat_rate: 23, sort_order: 1, tranche_label: 'Etap II' }] },
  ],
  contracts: [
...
export const demoDb = {
  ...
  invoices: {
    ...
    createFromProject(companyId: string, config: { projectId: string; vatRate?: number; tranches?: Array<{ id: string; label: string; amount: number; due_date: string }> }) { const { projectId, vatRate = 23, tranches = [] } = config; const state = read(); const project = state.projects.find((item) => item.id === projectId && item.company_id === companyId); if (!project) throw new Error('Project not found'); const items = tranches.length > 0 ? tranches.map((tr, idx) => ({ id: generateId(), description: tr.label || `Transza ${idx + 1}`, unit: 'kpl', quantity: 1, unit_price: tr.amount, vat_rate: vatRate, sort_order: idx + 1, tranche_label: tr.label || `Transza ${idx + 1}` })) : [{ id: generateId(), description: `Rozliczenie projektu ${project.name}`, unit: 'kpl', quantity: 1, unit_price: 0, vat_rate: vatRate, sort_order: 1, tranche_label: '' }]; return demoDb.invoices.create({ company_id: companyId, client_id: project.client_id, project_id: project.id, contract_id: null, status: 'unpaid', issue_date: dateOnly(), due_date: plusDays(7).slice(0,10), ksef_status: 'ksef_pending', ksef_ref: null, notes: `Faktura z projektu ${project.number}`, items }) },
  },
  contracts: {
