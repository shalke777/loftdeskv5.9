...
export const demoDb = {
  ...
  invoices: {
    ...
    createFromProject(companyId: string, config: { projectId: string; vatRate?: number; tranches?: Array<{ id: string; label: string; amount: number; due_date: string }> }) { const { projectId, vatRate = 23, tranches = [] } = config; const state = read(); const project = state.projects.find((item) => item.id === projectId && item.company_id === companyId); if (!project) throw new Error('Project not found'); const items = tranches.length > 0 ? tranches.map((tr, idx) => ({ id: crypto.randomUUID(), description: tr.label || `Transza ${idx + 1}`, unit: 'kpl', quantity: 1, unit_price: tr.amount, vat_rate: vatRate, sort_order: idx + 1, tranche_label: tr.label || `Transza ${idx + 1}` })) : [{ id: crypto.randomUUID(), description: `Rozliczenie projektu ${project.name}`, unit: 'kpl', quantity: 1, unit_price: 0, vat_rate: vatRate, sort_order: 1, tranche_label: '' }]; return demoDb.invoices.create({ company_id: companyId, client_id: project.client_id, project_id: project.id, contract_id: null, status: 'unpaid', issue_date: dateOnly(), due_date: plusDays(7).slice(0,10), ksef_status: 'ksef_pending', ksef_ref: null, notes: `Faktura z projektu ${project.number}`, items }) },
  },
  contracts: {
  ...
  projects: {
    ...
    create(input: Omit<Project, 'id' | 'number' | 'created_at'>) { const state = read(); const record: Project = { ...input, id: crypto.randomUUID(), number: nextNumber('PRJ', state.projects.filter((x) => x.company_id === input.company_id).length), created_at: now() }; state.projects.unshift(record); writeState(state); return record },
