...
const SCOPE_STATUS_COLOR: Record<AiScopeItem['review_status'], string> = {
  ...
  accepted: 'var(--color-success, var(--color-brand))',
  modified: 'var(--color-warning, var(--color-accent))',
  rejected: 'var(--color-danger, var(--color-error))',
}
...
const SEVERITY_COLOR: Record<AiRisk['severity'], string> = {
  high:   'var(--color-danger, var(--color-error))',
  medium: 'var(--color-warning, var(--color-accent))',
  low:    'var(--color-success, var(--color-brand))',
}
...
const Q_SEVERITY_COLOR: Record<AiQuestion['severity'], string> = {
  critical_for_scope:     'var(--color-danger, var(--color-error))',
  important_for_accuracy: 'var(--color-warning, var(--color-accent))',
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
        <div>
          ...
          {(() => {
            ...
            if (mr.best?.tier === 'strong') return <span title={`Katalog: ${mr.best.canonical_name}`} style={{ marginLeft: 4, fontSize: 9, padding: '0 4px', borderRadius: 3, background: 'var(--color-success-soft, var(--color-success-soft))', color: 'var(--color-success, var(--color-brand))', fontWeight: 600 }}>📚</span>
            ...
            return <span title="Brak dopasowania do katalogu" style={{ marginLeft: 4, fontSize: 9, padding: '0 4px', borderRadius: 3, background: 'var(--color-surface-soft, var(--color-surface-soft))', color: 'var(--color-text-tertiary, var(--color-text-muted))', fontWeight: 500 }}>✍️</span>
          })()}
          {item.scope_layer === 'HIDDEN_PROBABLE_SCOPE' && (
          <span style={{ marginLeft: 6, fontSize: 11, color: 'var(--color-warning, var(--color-accent))' }}>
            ukryty zakres
          </span>
        )}
        </div>
      ...
      <div style={{ fontSize: 12, color: 'var(--color-text-secondary)', display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        ...
        {item.missing_price && (
          <span style={{ color: 'var(--color-warning, var(--color-accent))' }}>brak ceny</span>
        )}
      </div>
...
function QuestionRow({
  ...
  return (
    ...
    >
      ...
      {answered
        ? (
          <p style={{ fontSize: 12, color: 'var(--color-success, var(--color-brand))', margin: 0 }}>
...
const COLOR_MAP: Record<string, { border: string; text: string; bg?: string }> = {
  success:   { border: 'var(--color-success, var(--color-brand))',         text: 'var(--color-success, var(--color-brand))' },
  warning:   { border: 'var(--color-warning, var(--color-accent))',         text: 'var(--color-warning, var(--color-accent))' },
  danger:    { border: 'var(--color-danger, var(--color-error))',          text: 'var(--color-danger, var(--color-error))' },
  ...
  if (run.status === 'failed') {
    ...
    return (
      <div style={{ padding: '16px 0' }}>
        <p style={{ color: 'var(--color-danger, var(--color-error))', fontSize: 13 }}>
  ...
  return (
    <div style={{ display: 'grid', gap: 20 }}>
      ...
      <div style={{
        ...
        <StatBadge label="Zaakceptowane" count={acceptedCount} color="var(--color-success, var(--color-brand))" />
        <StatBadge label="Zmodyfikowane" count={modifiedCount} color="var(--color-warning, var(--color-accent))" />
        <StatBadge label="Odrzucone" count={rejectedCount} color="var(--color-danger, var(--color-error))" />
        {pendingQuestions > 0 && <StatBadge label="Pytania" count={pendingQuestions} color="var(--color-primary, var(--color-info))" />}
        {openRisks > 0 && <StatBadge label="Ryzyka" count={openRisks} color="var(--color-danger, var(--color-error))" />}
      </div>
      ...
      >
        ...
        {run.missing_data && (
          <div style={{
            ...
            <span style={{ fontSize: 12, color: 'var(--color-warning, var(--color-accent))' }}>
      ...
      {run.status === 'completed' && (run.request_duration_ms || run.retry_count || run.model_name) && (
        ...
        >
          ...
          {run.retry_count != null && run.retry_count > 0 && (
            <span style={{ color: 'var(--color-warning, var(--color-accent))' }}>🔄 {run.retry_count}× retry</span>
          )}
          {run.timeout_occurred && (
            <span style={{ color: 'var(--color-danger, var(--color-error))' }}>⚠ timeout</span>
          )}
          {run.parse_path && (
          ...
          {run.input_file_size_bytes != null && run.input_file_size_bytes > 0 && (
            <span style={run.input_file_size_bytes > 5242880 ? { color: 'var(--color-warning, var(--color-accent))', fontWeight: 600 } : undefined}>
      ...
      <Section title="Zakres prac" count={scope.length}>
        ...
        {scope.length > 0 && (() => {
          ...
          return (
            <div style={{ fontSize: 11, color: 'var(--color-text-muted, var(--color-text-muted))', marginBottom: 6, display: 'flex', gap: 10 }}>
              <span>📚 Katalog: {matched}/{scope.length} ({rate}%)</span>
              {rate < 50 && <span style={{ color: 'var(--color-warning, var(--color-accent))' }}>⚠ niska pokrywalność</span>}
            </div>
          )
        })()}
        {scope.map(item => (
      ...
      {run.status === 'completed' && !lScope && (() => {
        ...
        return (
          ...
          >
            <span style={{ color: 'var(--color-success, var(--color-brand))' }}>
              ✓ {acceptedN + modifiedN} zaakceptowanych
              {modifiedN > 0 && (
                <span style={{ color: 'var(--color-warning, var(--color-accent))', marginLeft: 4 }}>
                  ({modifiedN} zmodyfikowanych)
                </span>
              )}
            </span>
            <span style={{ color: 'var(--color-danger, var(--color-error))' }}>
          ...
          >
            <div>
              ...
              <p style={{ fontSize: 12, color: 'var(--color-text-secondary)', margin: 0 }}>
                ...
                {missingPrices > 0 && (
                  <span style={{ color: 'var(--color-warning, var(--color-accent))', marginLeft: 6 }}>
            ...
            {existingEstimate
              ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <p style={{ fontSize: 13, color: 'var(--color-success, var(--color-brand))', margin: 0 }}>
            ...
            {createEstimate.isError && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <p style={{ fontSize: 12, color: 'var(--color-danger, var(--color-error))', margin: 0 }}>
                  {(() => {
      ...
      {scope.length > 0 && scope.every(s => s.review_status !== 'pending') && pendingQuestions === 0 && openRisks === 0 && (
        <div style={{
          ...
          fontSize: 13, color: 'var(--color-success, var(--color-brand))',
        }}>
