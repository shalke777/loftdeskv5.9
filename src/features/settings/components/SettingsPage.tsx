      ...
      <div className="grid-2">
        <Card>
          ...
          <div style={{ display: 'grid', gap: 6, fontSize: 14, marginBottom: 12 }}>
            <div style={{ display: 'flex', gap: 8 }}><span style={{ color: '#A7ABB3', minWidth: 80 }}>Użytkownik</span><strong>{user?.fullName || '—'}</strong></div>
            <div style={{ display: 'flex', gap: 8 }}><span style={{ color: '#A7ABB3', minWidth: 80 }}>E-mail</span><span>{user?.email || '—'}</span></div>
            <div style={{ display: 'flex', gap: 8 }}><span style={{ color: '#A7ABB3', minWidth: 80 }}>Rola</span><span>{user?.role || '—'}</span></div>
            <div style={{ display: 'flex', gap: 8 }}><span style={{ color: '#A7ABB3', minWidth: 80 }}>Firma</span><span>{user?.companyName || '—'}</span></div>
            <div style={{ display: 'flex', gap: 8 }}><span style={{ color: '#A7ABB3', minWidth: 80 }}>Plan</span><span>{user?.plan ? PLAN_DEFS[user.plan].name : '—'}</span></div>
          </div>
          <p style={{ fontSize: 13, color: '#A7ABB3', marginBottom: 12 }}>
        ...
        <Card>
          <h3>KSeF</h3>
          <div style={{ display: 'grid', gap: 6, fontSize: 14, marginBottom: 12 }}>
            <div style={{ display: 'flex', gap: 8 }}><span style={{ color: '#A7ABB3', minWidth: 120 }}>Środowisko</span><span>{(profile as any)?.ksef_env ?? 'test'}</span></div>
            <div style={{ display: 'flex', gap: 8 }}><span style={{ color: '#A7ABB3', minWidth: 120 }}>NIP do KSeF</span><span>{(profile as any)?.ksef_nip ?? 'brak — ustaw w danych wykonawcy'}</span></div>
            <div style={{ display: 'flex', gap: 8 }}><span style={{ color: '#A7ABB3', minWidth: 120 }}>Token</span><span>{(profile as any)?.ksef_token ? '✅ ustawiony' : '❌ brak — ustaw poniżej'}</span></div>
          </div>
          <p style={{ fontSize: 13, color: '#A7ABB3', marginBottom: 12 }}>{canUseKsef ? 'Integracja z KSeF aktywna w Twoim planie.' : 'Integracja z KSeF dostępna od planu Business. Przejdź na wyższy plan, aby wysyłać faktury elektronicznie.'}</p>
          <div className="actions-row">
            {canUseKsef
