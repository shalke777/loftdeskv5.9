        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            {logoUrl ? (
              ...
              <div style={{ width: 200, height: 80, borderRadius: 8, border: '2px dashed #d1d5db', display: 'grid', placeItems: 'center', color: '#9ca3af', fontSize: 13 }}>
                Brak logo
              </div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {logoUrl && (
                <Button disabled={!canEdit} onClick={handleLogoRemove} style={{ background: 'transparent', color: '#ef4444', border: '1px solid #fca5a5' }}>
                  Usuń logo
                </Button>
              )}
              <p style={{ margin: 0, color: '#9ca3af', fontSize: 12 }}>PNG, JPG, SVG lub WebP · maks. 500 KB</p>
              {logoError && <p style={{ margin: 0, color: '#ef4444', fontSize: 12 }}>{logoError}</p>}
            </div>
          </div>
        </div>
      )}
