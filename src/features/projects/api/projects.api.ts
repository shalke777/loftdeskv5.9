...
export const projectsApi = {
  async list(companyId: string): Promise<Project[]> {
    ...
    return (data ?? []).map((row: any) => ({ id: row.id, company_id: row.company_id ?? companyId, client_id: row.client_id, number: row.number, name: row.name, status: row.status, start_date: row.start_date, end_date: row.end_date, address: row.address ?? '', investment_address: row.investment_address ?? null, budget: Number(row.budget ?? 0), costs: Number(row.costs ?? 0), notes: row.notes ?? '', completeness_score: Number(row.completeness_score ?? 0), completeness_flags: row.completeness_flags ?? null, archived_at: row.archived_at ?? null, created_at: row.created_at }))
  },
  async create(input: CreateProjectInput): Promise<Project> {
    ...
    const payload = withScope(scope, { number: `PRJ/${new Date().getFullYear()}/${Date.now().toString().slice(-4)}`, client_id: input.client_id, name: input.name, status: input.status, start_date: input.start_date, end_date: input.end_date, address: input.address ?? null, investment_address: input.investment_address ?? null, budget: input.budget ?? null, costs: input.costs ?? 0, notes: input.notes ?? null, completeness_score: 0, completeness_flags: {} })
    const { data, error } = await supabase.from('projects').insert(payload).select('*').single()
    if (error) throw error
    return { id: data.id, company_id: data.company_id ?? input.company_id, client_id: data.client_id, number: data.number, name: data.name, status: data.status, start_date: data.start_date, end_date: data.end_date, address: data.address ?? '', investment_address: data.investment_address ?? null, budget: Number(data.budget ?? 0), costs: Number(data.costs ?? 0), notes: data.notes ?? '', completeness_score: Number(data.completeness_score ?? 0), completeness_flags: data.completeness_flags ?? null, archived_at: data.archived_at ?? null, created_at: data.created_at }
  },
  ...
  async createFromEstimate(companyId: string, estimateId: string) {
    ...
    return { id: data.id, company_id: data.company_id ?? companyId, client_id: data.client_id, number: data.number, name: data.name, status: data.status, start_date: data.start_date, end_date: data.end_date, address: data.address ?? '', investment_address: data.investment_address ?? null, budget: Number(data.budget ?? 0), costs: Number(data.costs ?? 0), notes: data.notes ?? '', completeness_score: data.completeness_score ?? 0, completeness_flags: data.completeness_flags ?? {}, created_at: data.created_at }
  },
