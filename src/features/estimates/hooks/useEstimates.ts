...
export function useCreateEstimate() { const companyId = useCompanyId(); const queryClient = useQueryClient(); const toast = useToast(); return useMutation({ mutationFn: estimatesApi.create, onSuccess: (data) => { queryClient.invalidateQueries({ queryKey: estimateKeys.list(companyId) }); queryClient.invalidateQueries({ queryKey: ['onboarding-progress', companyId] }); toast.success('Kosztorys utworzony'); autoLinkService.link({ type: 'estimate', id: data.id, companyId, clientId: data.client_id, projectId: data.project_id ?? null }).catch((err) => console.warn('[autoLink] estimate link failed:', err)) }, onError: (error) => toast.error('Nie udało się utworzyć kosztorysu', translateError(error)) }) }
export function useUpdateEstimate() { const companyId = useCompanyId(); const queryClient = useQueryClient(); const toast = useToast(); return useMutation({ mutationFn: ({ id, input }: { id: string; input: Partial<Estimate> }) => estimatesApi.update(id, input, companyId), onSuccess: () => { queryClient.invalidateQueries({ queryKey: estimateKeys.list(companyId) }); toast.success('Kosztorys zaktualizowany') }, onError: (err: any) => { toast.error('Błąd zapisu wyceny', err?.message ?? 'Sprawdź dane i spróbuj ponownie') } }) }
export function useDeleteEstimate() { const companyId = useCompanyId(); const queryClient = useQueryClient(); const toast = useToast(); return useMutation({ mutationFn: (id: string) => estimatesApi.delete(id, companyId), onSuccess: () => { queryClient.invalidateQueries({ queryKey: estimateKeys.list(companyId) }); queryClient.invalidateQueries({ queryKey: ['project_documents'] }); toast.info('Kosztorys usunięty') }, onError: (error) => toast.error('Nie udało się usunąć kosztorysu', translateError(error)) }) }
