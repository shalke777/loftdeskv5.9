...
export function useUpdateEstimate() { const companyId = useCompanyId(); const queryClient = useQueryClient(); const toast = useToast(); return useMutation({ mutationFn: ({ id, input }: { id: string; input: Partial<Estimate> }) => estimatesApi.update(id, input, companyId), onSuccess: (data) => { const updated = data as Estimate; queryClient.setQueryData<Estimate[]>(estimateKeys.list(companyId), (old = []) => old.map(e => e.id === updated.id ? updated : e)); queryClient.invalidateQueries({ queryKey: estimateKeys.list(companyId) }); /* P0-5 fix: also refresh the detail cache so re-opening the edit modal shows the latest data, not stale 30s-old detail that wipes recent changes. */ queryClient.setQueryData(['estimates', 'detail', companyId, updated.id], updated); queryClient.invalidateQueries({ queryKey: ['estimates', 'detail', companyId, updated.id] }); toast.success('Kosztorys zaktualizowany'); if (updated.project_id) { autoLinkService.link({ type: 'estimate', id: updated.id, companyId, clientId: updated.client_id, projectId: updated.project_id }).then(() => { queryClient.invalidateQueries({ queryKey: ['projects', companyId] }) }).catch((err) => console.warn('[autoLink] estimate update link failed:', err)) } }, onError: (error: any) => { const msg = error?.message ?? error?.details ?? 'Sprawdź połączenie i spróbuj ponownie'; toast.error('Błąd zapisu wyceny', msg); console.error('[estimates] update error:', error) } }) }
export function useDeleteEstimate() { const companyId = useCompanyId(); const queryClient = useQueryClient(); const toast = useToast(); return useMutation({ mutationFn: (id: string) => estimatesApi.delete(id, companyId), onMutate: (id) => { queryClient.setQueryData<Estimate[]>(estimateKeys.list(companyId), (old = []) => old.filter(e => e.id !== id)); return { id } }, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['project_documents'] }); toast.info('Kosztorys usunięty') }, onError: (_error, _id, context) => { if (context?.id) queryClient.invalidateQueries({ queryKey: estimateKeys.list(companyId) }); toast.error('Nie udało się usunąć kosztorysu') } }) }
