...
export function useDeleteEstimate() { const companyId = useCompanyId(); const queryClient = useQueryClient(); const toast = useToast(); return useMutation({ mutationFn: estimatesApi.delete, onSuccess: () => { queryClient.invalidateQueries({ queryKey: estimateKeys.list(companyId) }); toast.info('Kosztorys usunięty') } }) }
