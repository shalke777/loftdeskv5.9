...
export const clientsApi = {
  ...
  async create(input: Omit<Client, 'id' | 'created_at'>): Promise<Client> {
    ...
    const payload = withScope(scope, { name: input.name, email: input.email || null, phone: input.phone || null, city: input.city || null, address: input.address || null, postal_code: input.postal_code || null, nip: input.nip || null, pesel: input.pesel || null, contact_person: input.contact_person || null })
    const { data, error } = await supabase.from('clients').insert(payload).select('*').single()
    if (error) throw error
    return { id: data.id, company_id: data.company_id ?? input.company_id, name: data.name, email: data.email ?? '', phone: data.phone ?? '', city: data.city ?? '', address: data.address ?? '', postal_code: data.postal_code ?? '', nip: data.nip ?? '', pesel: data.pesel ?? '', contact_person: data.contact_person ?? '', created_at: data.created_at }
  },
  async update(id: string, input: Partial<Client>, companyId?: string): Promise<Client | null> {
    ...
    const { data, error } = await query
    if (error) throw error
    return { id: data.id, company_id: data.company_id ?? companyId ?? '', name: data.name, email: data.email ?? '', phone: data.phone ?? '', city: data.city ?? '', address: data.address ?? '', postal_code: data.postal_code ?? '', nip: data.nip ?? '', pesel: data.pesel ?? '', contact_person: data.contact_person ?? '', created_at: data.created_at }
  },
