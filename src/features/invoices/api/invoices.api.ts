...
export const invoicesApi = {
  ...
  async createCorrection(companyId: string, originalInvoiceId: string): Promise<Invoice> {
    ...
    // Fetch client data for the original_data snapshot
    let clientSnapshot: { client_name: string | null; client_nip: string | null; client_address: string | null } = { client_name: null, client_nip: null, client_address: null }
    if (original.client_id) {
      const { data: client } = await supabase.from('clients').select('name, nip, address').eq('id', original.client_id).single()
      if (client) {
        clientSnapshot = { client_name: client.name ?? null, client_nip: client.nip ?? null, client_address: client.address ?? null }
      }
    }
    ...
    // original_data = snapshot of header fields before correction
    const originalData: Record<string, string | null> = {
      client_id: original.client_id ?? null,
      ...clientSnapshot,
      invoice_number: original.number ?? null,
      issue_date: original.issue_date ?? null,
