...
export function useDeleteInvoice() { const companyId = useCompanyId(); const qc = useQueryClient(); const toast = useToast(); return useMutation({ mutationFn: (id: string) => invoicesApi.delete(id, companyId), onMutate: (id) => { qc.setQueryData<Invoice[]>(invoiceKeys.list(companyId), (old = []) => old.filter(i => i.id !== id)); return { id } }, onSuccess: () => { qc.invalidateQueries({ queryKey: ['dashboard', companyId] }); qc.invalidateQueries({ queryKey: ['project_documents'] }); toast.info('Faktura usunięta') }, onError: (_error, _id, context) => { if (context?.id) qc.invalidateQueries({ queryKey: invoiceKeys.list(companyId) }); toast.error('Nie udało się usunąć faktury') } }) }
